-- Daily View dashboard/display: countdown to the next event.
-- Adds a minutesUntil field to nextEvent so the client can show a rounded,
-- low-anxiety countdown ("in about 10 minutes") next to the existing exact
-- time, matching the wording already shipped on the marketing site's static
-- mockups. Computed server-side (rather than parsed from timeLabel
-- client-side) because timeLabel is locale/12-24h formatted text, and
-- "now" vs. the event's start_time are both already resolved here in the
-- account's timezone in one place.
--
...
-- ============================================================================
-- dv_get_today_view_model(): add nextEvent.minutesUntil
-- ============================================================================
...
-- 20260707090000_dv_display_viewer.sql, and 20260711120000_dv_time_format.sql,
-- which did the same). The only change is the added 'minutesUntil' key in
-- the v_next jsonb_build_object, computed via extract(epoch from ...)/60 on
-- the interval between start_time and v_local_time (both already-resolved
-- local `time` values, so no timezone handling is needed here).
-- dv_get_viewer_snapshot() (the real display's entry point) needs no change
-- of its own -- it delegates to this function and returns its jsonb as-is.
