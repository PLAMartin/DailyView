-- Daily View dashboard/display: "coming up" advance reminder for recurring events.
-- Adds an `upcomingReminder` field to dv_get_today_view_model()'s returned jsonb: the single
-- nearest recurring-series event (series_id is not null -- i.e. birthdays/anniversaries created
-- via dv_event_series, not one-off appointments) whose occurrence falls within the account's
-- `upcoming_reminder_lead_days` window, strictly after today (today's own occurrence is already
-- covered by `events`/`nextEvent`). Deliberately a single item, not a list, to keep the display
-- calm -- matches the site's own "fewer, larger, clearer elements" design ethos.
--
-- dv_get_viewer_snapshot() needs no change of its own -- confirmed it delegates to this function
-- and returns its jsonb as-is (supabase/migrations/20260707090000_dv_display_viewer.sql).
--
...
-- ============================================================================
-- dv_get_today_view_model(): add upcomingReminder
-- ============================================================================
-- Full function body reproduced (create or replace requires it), matching this repo's
-- established pattern (see 20260706140000_dv_display_message.sql, 20260707090000_dv_display_viewer.sql,
-- 20260711120000_dv_time_format.sql and 20260726190000_dv_next_event_countdown.sql, which did
-- the same). The only change is the added v_upcoming computation and its 'upcomingReminder' key
-- in the final jsonb_build_object -- it reuses the already-resolved v_status_cancelled_id,
-- v_visibility_display_id, v_local_date and v_time_fmt rather than recomputing them.
...
begin
  ...
  where e.account_id = p_account_id
    ...
    and e.event_status_id is distinct from v_status_cancelled_id
    and e.event_date > v_local_date
    and e.event_date <= v_local_date + coalesce(v_account.upcoming_reminder_lead_days, 3)
  order by e.event_date asc, e.display_priority asc
  limit 1;
